...
function mostCommonGenres(books) {
  ...
  let newObjectList = filteredGenreList.reduce((acc, genre_f) => {
    let newObject = { // when your object is declared over multiple lines, place each parameter on a new line for improved readability
      name: genre_f,
      count: books.reduce((count, book) => {
        if (book.genre === genre_f) { // it's recommended to always use brackets for your conditions and loops so the code is easier to follow
          count += 1;
        }
        return count;
      }, 0),
