...
// should use let instead of var to declare variables
let left = -1;
let right = 1;
function sortAccountsByLastName(accounts) {
  return accounts.sort(
    (a,b) => a.name.last<b.name.last ? left : right
  );
}
...
function booksInPossession(account, books, authors) {
  let arr = [];
  for (let index in books) {
    let booksOut = books[index]
      .borrows
      .some(book => (book.id === account.id && book.returned === false));
      if (booksOut) {
